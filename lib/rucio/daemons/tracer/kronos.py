...
class AMQConsumer(object):
    ...
    def on_error(self, headers, message):
        ...
        print('[%s] %s' % (self.__broker, message))

    def on_message(self, headers, message):
        f2()
        print('on_messages ... YYG')
        ...
        if 'resubmitted' in headers:
            ...
            print('(kronos_file on message) got a resubmitted report')
        ...
        try:
            if appversion == 'dq2':
                ...
                print("message of on_messages: " + message )      
        except Exception:
        ...
        if len(self.__ids) >= self.__chunksize:
            ...
            self.__update_atime()
            print('(kronos_file on_messages ) done calling_upate_atime() ... YYG')
    ...
    def __update_atime(self):
        ...
        """
        f2()
        print('(__upate_atime()) just got in  ... YYG')
        replicas = []
        rses = []
        for report in self.__reports:
            try:
                        ...
                        for pattern in self.__bad_files_patterns:
                            if 'stateReason' in report and report['stateReason'] and isinstance(report['stateReason'], str) and pattern.match(report['stateReason']):
                                ...
                                if 'url' not in report or not report['url']:
                                    print('Missing url in the following trace : ' + str(report))
                                else:
                                    try:
                                        surl = report['url']
                                        declare_bad_file_replicas([surl, ], reason=reason, issuer=InternalAccount('root'), status=BadFilesStatus.SUSPICIOUS)
                                        print('Declare suspicious file %s with reason %s' % (report['url'], reason))
                                    except Exception as error:
                                        print('Failed to declare suspicious file' + str(error))
                except Exception as error:
                    print('Problem with bad trace : %s . Error %s' % (str(report), str(error)))
                ...
                if report['eventType'] != 'touch':
                    ...
                    for rse in rses:
                        ...
                        print('(__upate_atime()) update replicas  .. YYG')
                    ...
                    if 'remoteSite' in report:
                        ...
                        rse_id = get_rse_id(rse=rse)
                        print('(__upate_atime(), res_id %d'%res_id) 
                    if 'datasetScope' in report:
                        print('(__upate_atime()) update __dataset_queue while datasetScope in report .. YYG')
                        ...
                        if 'remoteSite' not in report:
                            continue
                        print('(__upate_atime()) update replicas 2  .. YYG')
                        replicas.append({'name': report['filename'], 'scope': report['scope'], 'rse': rse, 'rse_id': rse_id, 'accessed_at': datetime.utcfromtimestamp(report['traceTimeentryUnix'])})
                        print(replicas)
            ...
            for did in list_parent_dids(report['scope'], report['filename']):
                ...
                for rse in rses:
                    rse_id = get_rse_id(rse=rse)
                    print('(__upate_atime()) update __dataset_queue')
                    ...
                    f2()
        print('checking replicas in __update_atime')
        print(replicas)
        ...
        try:
            ...
            for replica in replicas:
                ...
                print('(__upate_atime()) update a replica for accessing time')
                if not touch_replica(replica):
                    ...
                    print('(kronos_file) hit locked row, resubmitted to queue')
        ...
        print('(kronos_file) updated %d replicas' % len(replicas))
...
def kronos_file(once=False, thread=0, brokers_resolved=None, dataset_queue=None, sleep_time=60):
    ...
    print('(kronos_file) thread: %s tracer consumer starting'%threadN)
    f2()
    ...
    try:
        ...
        print('(kronos_file) Failed to get bad_file_patterns' + str(error))
        bad_files_patterns = []
    ...
    print('(kronos_file) tracer consumer started')
    ...
    while not graceful_stop.is_set():
        ...
        for conn in conns:
            if not conn.is_connected():
                print('(kronos_file thread: %s ) connecting to %s' % (thread.getName(), conn.transport._Transport__host_and_ports[0][0]))
                ...
                conn.start()
                f2()
                print("connection started... YYG")
                if not use_ssl:
                    f2()
                    print("To be connected with pd ... YYG")
                    conn.connect(username, password)
                    print("connected with pd ... YYG")
                else:
                    conn.connect()
    ...
    print('(kronos_file) graceful stop requested')

    for conn in conns:
        try:
            f2()
            print('(kronos_file) disconnected  ... YYG')
            conn.disconnect()
        except Exception:
            pass

    die(executable='kronos-file', hostname=hostname, pid=pid, thread=thread)
    print('(kronos_file) graceful stop done')
...
def kronos_dataset(once=False, thread=0, dataset_queue=None, sleep_time=60):
    ...
    print('(kronos_dataset) starting2')
    print('(kronos_dataset) dataset_wait: %d '%dataset_wait)
    start = datetime.now()
    print('(kronos_dataset) start: %s' %start)
    ...
    while not graceful_stop.is_set():
        ...
        tottime = time() - start_time
        if tottime < sleep_time:
            print('(kronos_dataset) Will sleep for %s seconds' % (sleep_time - tottime))
            sleep(sleep_time - tottime)
    # once again for the backlog
    f2()
    die(executable='kronos-dataset', hostname=hostname, pid=pid, thread=thread)
    print('(kronos_dataset) cleaning dataset backlog before shutdown...')
    print('kronos_dataset: to be __update_dataset 2 -- YYG ')
    __update_datasets(dataset_queue)


def __update_datasets(dataset_queue):
    f2()
    print('(__update_dataset) starting YYG') 
    len_ds = dataset_queue.qsize()
    print('(__update_datasets) dataset_queue.qsize() : %d'%len_ds)
    datasets = {}
    dslocks = {}
    now = time()
    for _ in range(0, len_ds):
        ...
        print('(__update_datasets) did: %s ' %did)
        rse = dataset['rse_id']
        if did not in datasets:
            f2()
            datasets[did] = dataset['accessed_at']
            print('(__update_datasets) 1: datasets[did]: %s' %datasets[did])
        else:
            f2()
            datasets[did] = max(datasets[did], dataset['accessed_at'])
            print('(__update_datasets) 2: datasets[did]: %s' %datasets[did])
    ...
    print('(__update_datasets) fetched %d datasets from queue (%ds)' % (len_ds, time() - now))
    ...
    for did, accessed_at in datasets.items():
        ...
        print('__update_datasets: to be touch_dids -- YYG')
    ...
    for did, rses in dslocks.items():
        ...
        for rse, accessed_at in rses.items():
            ...
            if not touch_dataset_locks((update_dslock,)):
                ...
                failed += 1
            total += 1
    print('(__update_dataset) did update for %d locks, %d failed (%ds)' % (total, failed, time() - start))
    ...
    for did, rses in dslocks.items():
        ...
        for rse, accessed_at in rses.items():
            ...
            update_dslock = {'scope': scope, 'name': name, 'rse_id': rse, 'accessed_at': accessed_at}
            # if update fails, put back in queue and retry next time
            print('__update_datasets: to be touch_collection_replicas -- YYG')
            if not touch_collection_replicas((update_dslock,)):
                f2()
                dataset_queue.put(update_dslock)
                failed += 1
                print('__update_datasets: failed touch_collection_replicas -- YYG')
            total += 1
    print('(__update_dataset) did update for %d collection replicas, %d failed (%ds)' % (total, failed, time() - start))
...
def run(once=False, threads=1, sleep_time_datasets=60, sleep_time_files=60):
    ...
    print('resolving broker dns alias: %s' % brokers_alias)
    ...
    print('brokers resolved to %s', brokers_resolved)

    dataset_queue = Queue()
    print('starting tracer consumer threads')
